function NARWVocalPitchEnergyV0_4_English()
...
% Author: Yu Shiu
    ...
    if exist('ax1', 'var')
        ...
        ax1 = axes('Parent', f1, 'position', [0.10, 0.30, 0.85, 0.65]);
    end

    % Filename display
    %UiFilename = uicontrol('Style','text','Position',[20 5 400 20]);
    UiFilename = uicontrol('Style','text','Position',[520 5 400 20]);
    ...
    b12 = uicontrol('Style','popup','Position',[520,5+25*1+10,140,20], 'String', {'Choose Score:','1: Good','2: Mediocre','3: Bad'} ,'Callback', @StoreScore); 
    ...
    uicontrol('Style','pushbutton','Position',[720+80,5+25*2+10,60,20],'String','Next','Callback',{@NextSound, SoundList, SoundFolder, p, p2, f1, ax1, UiFilename, FreqRange});
    ...
    uicontrol('Style','pushbutton','Position',[720,5+25*2+10,60,20],'String','Reset','Callback',{@ResetSound, SoundList, SoundFolder, p, p2, f1, ax1, UiFilename, FreqRange});
    ...
    uicontrol('Style','pushbutton','Position',[720,5+25*1+10,140,20],'String','End','Callback',{@TermiTask, SoundList, p, p2});
    ...
    uicontrol('Style','pushbutton','Position',[520,5+25*2+10,140,20],'String','Play', 'Callback', {@PlayCallback});

    %% ReDrawWeight1
    %global Weight0;
    %PitchRange = p.flim;
    % Slider for weights of pitch estimation algorithm
    uicontrol('Style','text','Position',[20 5+25*2 50 20],'String','Weight 1');
    b1 = uicontrol('Parent',f1,'Style','slider','Position',[80,5+25*2,350,20],'min', 0.0, 'max',30.0, 'Value',15.0);
    b1.Callback = @(es,ed) ReDrawWeight1(1, es.Value);

    uicontrol('Style','text','Position',[20 5+25*1 50 20],'String','Weight 2');
    b2 = uicontrol('Parent',f1,'Style','slider','Position',[80,5+25*1,350,20],'min', 0.0, 'max',20.0, 'Value',10.0);
    b2.Callback = @(es,ed) ReDrawWeight1(2, es.Value);

    uicontrol('Style','text','Position',[20 5+25*0 50 20],'String','Weight 3');
    b3 = uicontrol('Parent',f1,'Style','slider','Position',[80,5+25*0,350,20],'min', 0.0, 'max',20.0, 'Value',10.0);
    b3.Callback = @(es,ed) ReDrawWeight1(3, es.Value);

    %% Currnet pitch range
    b10 = uicontrol('Style','text','Position',[225 5+25*3 30 20],'String',num2str(p.flim(1,1)));
    b11 = uicontrol('Style','text','Position',[260 5+25*3 30 20],'String',num2str(p.flim(1,2)));
    ...
    uicontrol('Style','text','Position',[20 5+25*3 80 20],'String','F0 Range', 'HorizontalAlignment','left');
    b4 = uicontrol('Parent',f1,'Style','slider','Position',[80,5+25*3,140,20],'min', 0, 'max',500.0, 'Value', p.flim(1,1), 'SliderStep', [1/500, 20/500]);
    FlagPitch = 0;
    b4.Callback = @(es,ed) ReDrawPitchRange(es.Value, FlagPitch);

    b5 = uicontrol('Parent',f1,'Style','slider','Position',[290,5+25*3,140,20],'min', 0, 'max',1000.0, 'Value', p.flim(1,2), 'SliderStep', [1/1000, 20/1000]);
    FlagPitch = 1;
    b5.Callback = @(es,ed) ReDrawPitchRange(es.Value, FlagPitch);

    %% Currnet time boundary
    b8 = uicontrol('Style','text','Position',[225 5+25*4 30 20],'String', num2str(0.0,'%2.3f'));
    b9 = uicontrol('Style','text','Position',[260 5+25*4 30 20],'String', num2str(1.0,'%2.3f'));
    
    %% Adjust Time Boundary: start time and end time
    SoundLength = 100;
    uicontrol('Style','text','Position',[20 5+25*4+20 160 20],'String','F0 Measurement:', 'HorizontalAlignment','left');
    uicontrol('Style','text','Position',[20 5+25*4 80 20],'String','Time Range', 'HorizontalAlignment','left');
    b6 = uicontrol('Parent',f1,'Style','slider','Position',[80,5+25*4,140,20],'min', 0, 'max', SoundLength, 'Value', IndL, 'SliderStep', [1/SoundLength, 10/SoundLength]);
    %FlagTime = 0; % Start Time
    b6.Callback = @(es,ed) RefineTimeBoundary(es.Value, 0);
    
    b7 = uicontrol('Parent',f1,'Style','slider','Position',[290,5+25*4,140,20],'min', 0, 'max', SoundLength, 'Value', IndU, 'SliderStep', [1/SoundLength, 10/SoundLength]);
    %FlagTime = 1; % End time
    b7.Callback = @(es,ed) RefineTimeBoundary(es.Value, 1);
    
    %% Currnet energy freq range
    b13 = uicontrol('Style','text','Position',[660 5+25*4 30 20],'String', num2str(EneFreqL,'%4.0f'));
    b14 = uicontrol('Style','text','Position',[690 5+25*4 30 20],'String', num2str(EneFreqU,'%4.0f'));
    
    %% Freq range for energy measurement
    uicontrol('Style','text','Position',[440 5+25*4+20 160 20],'String','Energy Measurement:', 'HorizontalAlignment','left');
    uicontrol('Style','text','Position',[440 5+25*4 80 20],'String','Freq Range', 'HorizontalAlignment','left');
    b15 = uicontrol('Parent',f1,'Style','slider','Position',[520,5+25*4,140,20],'min', 0, 'max', p.fmax, 'Value', EneFreqL, 'SliderStep', [10/p.fmax, 100/p.fmax]);
    b15.Callback = @(es,ed) RefineEneFreqRange(es.Value, 0);
    b16 = uicontrol('Parent',f1,'Style','slider','Position',[720,5+25*4,140,20],'min', 0, 'max', p.fmax, 'Value', EneFreqU, 'SliderStep', [10/p.fmax, 100/p.fmax]);
    b16.Callback = @(es,ed) RefineEneFreqRange(es.Value, 1);
    
    
    %% spectrogram axes
    % x-axis
    uicontrol('Style','text','Position',[440 5+25*4+45 160 20],'String','Time (Sec)', 'FontName', 'Arial', 'FontSize', 10, 'HorizontalAlignment','left');
    % y-axis
    uicontrol('Style','text','Position',[5 200 70 20],'String','Frequency', 'FontName', 'Arial', 'FontSize', 10, 'HorizontalAlignment','left');
    uicontrol('Style','text','Position',[5 180 70 20],'String','(Hz)', 'FontName', 'Arial', 'FontSize', 10, 'HorizontalAlignment','left');
    ...
    function TermiTaskCore
        ...
        MatPath = uigetdir(CurrFolder, 'End the analysis and save the results: select the output folder');
        OutputFileName = strcat('NARWPitch-',char(datestr(now,'yyyymmdd-HHMM')), '.mat');
        save(fullfile(MatPath, OutputFileName), 'PitchContour');
    end
    ...
    function ReDrawWeight1(VarIndex, SliderVal1)
        ...
        if PitchRange(1,1) < PitchRange(1,2)
            ...
            disp('F0 range low needs to be smaller than F0 range high!')
        end    
    end
    ...
    function [Fx, Tx, h1, h2, h3, h4, h5] = TargetCallCalc(SoundList, SoundFolder, CurrSound, p, p2, f1, UiFilename, FreqRange)
        ...
        set(b12, 'String', {'Choose Score:','1: Good','2: Mediocre','3: Bad'});
        set(b12, 'Value', 1);
