function [fx,tx,pv,fv]=fxpefacYuV3(s,fs,tinc,m,pp,Weight)
...
if ~numel(w_u)
    ...
    %dpwtdef=[1.0000, 0.8250, 0.01868, 0.006773, 200.0, -0.4238];
    %YuShiu's play
    %dpwtdef=[1.0000, 0.8250, 1.000, 1.0, 500.0, 0.0];
    dpwtdef=[0.6500, 20.0000, 1.000, 10000.0, 500.0, 0.0];
    %dpwtdef=[Weight(1), Weight(2), Weight(3), Weight(4), Weight(5), Weight(6)];
    %dpwtdef = Weight;
    %dpwtdef=[0.2500, 1.0000, 1.0000, 500.0, 1000.0, 0.5];
    %dpwtdef=[1.0000, 0.2000, 1.000, 500.0, 200.0, 0.0];
    %===== END
...
dpwtdef = Weight;
if 1
p1.fstep=2;              % frequency resolution of initial spectrogram (Hz)
%p1.fstep=10;
p1.fmax=2000;            % maximum frequency of initial spectrogram (Hz)
%p1.fres = 20;            % bandwidth of initial spectrogram (Hz)
p1.fres = 10;
p1.fbanklo = 10;         % low frequency limit of log filterbank (Hz)
p1.mpsmooth = 21;       % width of smoothing filter for mean power
p1.maxtranf = 1000;      % maximum value of tranf cost term
p1.shortut = 7;          % max utterance length to average power of entire utterance
p1.pefact = 1.8;         % shape factor in PEFAC filter
%p1.numopt = 3;           % number of possible frequencies per frame
p1.numopt = 50;
%p1.flim = [60 400];      % range of feasible fundamental frequencies (Hz)
p1.flim = [30 1200];
%p1.flim = [30 800];
p1.w = dpwtdef;          % DP weights
% p1.rampk = 1.1;          % constant for relative-amplitude cost term
% p1.rampcz = 100;         % relative amplitude cost for missing peak
%p1.tmf = 2;              % median frequency smoothing interval (s)
p1.tmf = 0.2;
p1.tinc = 0.01;          % default frame increment (s)
p1.sopt = 'ilcwpf';      % spectrogram options
end
...
%if nargin>=4  && numel(tinc)>0
    p1.tinc = tinc;   % 0.01 s between consecutive time frames
end
if nargin<4
%if nargin<5
    m='';
...
fmin = 0; fstep = p1.fstep; fmax = p1.fmax;
fres = p1.fres;  % Frequency resolution (Hz)
[tx,f,MIX]=spgrambw(s,fs,fres,[fmin fstep fmax],[],p1.tinc);
nframes=length(tx);
...
% Log-frequency scale
[trans,cf]=filtbankm(length(f),2*length(f)-1,2*f(end),p1.fbanklo,f(end),'usl');
...
if tx(end)<p1.shortut                        % if it is a short utterance
    eltass = mean(O,1);                     % mean power per each frequency band
    eltass = smooth(eltass,p1.mpsmooth);     % smooth in log frequency
    ...
    eltass = timesm(O,stt);
    eltass = smooth(eltass,p1.mpsmooth);     % filter in time and log frequency
...
sca1 = sca;
filh = 1./(p1.pefact-cos(2*pi*sca1));
...
numopt = p1.numopt; % Number of possible fundamental frequencies per frame
flim = p1.flim;
...
for i=1:nframes
    ...
    [pos,peak]=v_findpeaks(B(i,pfreq),[],2.0/(cf(pfreq(2))-cf(pfreq(1))));
    if numel(pos)
...
if ~nargout || any(m=='g') || any(m=='G')
    ...
    if any(m=='G') || ~nargout && ~any(m=='g')
        clf;
        spgrambw(s,fs,p1.sopt); % draw spectrogram with log axes
        hold on
